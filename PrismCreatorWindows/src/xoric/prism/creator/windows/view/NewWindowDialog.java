...
public class NewWindowDialog implements ActionListener, INewDialog
{
	...
	public NewWindowDialog()
	{
		...
		nameField = new PrismTextField("NEW WINDOW");
		...
		pathLabel2.setToolTipText(ToolTipFormatter.split("This directory will be used to store the window's settings."));
	...
	@Override
	public NewWindowData getResult()
	{
		Text name = new Text(nameField.getText());
		Path path = new Path(pathField.getText());

		NewWindowData d = new NewWindowData(name, path);
	...
	{
		int n = JOptionPane.showConfirmDialog(null, message, "New Window", JOptionPane.OK_CANCEL_OPTION);
