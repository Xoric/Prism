...
public class InputHandler implements IUISubcomponent, IStackable
{
	...
	public void mouseDown(IFloatPoint_r mouse)
	{
		...
		while (i < displayLeft.length)
		{
			...
			wThis = AllTools.printer.getDefaultPadding(symbol);
	...
	private float trimWord(IText_r t, Word word, float width, boolean fromLeft)
	{
		...
		{
			w = AllTools.printer.calcTextWidth(t, word); // initial word width
			...
			while (w > width) // while the word's width is too large for the display area (width)
			{
				...
				w -= AllTools.printer.getDefaultPadding(TextMap.symbolOf(c, -1));
			}
		}
		return w;
	}
	...
	public void draw(IRendererUI renderer, boolean drawCursor) throws PrismException
	{
		...
		AllTools.printer.setOnset(displayLeft.start, displayLeft.calcEnd());
		float newX = AllTools.printer.print(tempRect.getTopLeft());
		tempRect.setX(newX);
		...
		if (drawCursor)
		{
			...
			AllTools.printer.setColor(BlinkColor.selectionColor.getMixedColor());
		}
		...
		AllTools.printer.setOnset(displayMiddle.start, displayMiddle.calcEnd());
		newX = AllTools.printer.print(tempRect.getTopLeft());
		tempRect.setX(newX);
		...
		//		r.setX(x);
		//		Materials.printer.setText(text);
		AllTools.printer.setOnset(displayRight.start, displayRight.calcEnd());
		newX = AllTools.printer.print(tempRect.getTopLeft());
		tempRect.setX(newX);
		...
		if (drawCursor)
		{
			...
			AllTools.printer.setText(cursorText);
			AllTools.printer.print(tempCursor);
		}
		...
		if (displayLeft.start > 0)
		{
			...
			//			Printer.printSingle(renderer, cursor, "<");
			AllTools.printer.setText(markLeft);
			AllTools.printer.print(tempCursor);
		}
		...
		if (displayRight.calcEnd() < text.length())
		{
			...
			//			Printer.printSingle(renderer, cursor, ">");
			AllTools.printer.setText(markRight);
			AllTools.printer.print(tempCursor);
		}
