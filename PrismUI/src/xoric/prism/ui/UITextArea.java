...
public class UITextArea implements IDrawableUI, IUITextComponent, IUISubcomponent
{
	...
	public void rearrangeLines()
	{
		...
		if (text != null)
		{
			...
			do
			{
				...
				float w = AllTools.printer.calcTextWidth(text, lastIndex, index);
				width += w;
	...
	public void draw(IRendererUI renderer) throws PrismException
	{
		...
		for (int i = 0; i < stops.size(); ++i)
		{
			...
			AllTools.printer.setOnset(start, j);
			AllTools.printer.print(tempPos);
			tempPos.y += AllTools.printer.getHeight();
