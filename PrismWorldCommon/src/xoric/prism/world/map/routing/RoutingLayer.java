...
public class RoutingLayer
{
	...
	public void init(RoutingTile[][] tiles)
	{
		int tileCountX = tiles[0].length;
		int tileCountY = tiles.length;
		nodes = new RoutingNode[tileCountY][tileCountX]; // TODO bugs
		...
		for (int iy = 0; iy < tiles.length; ++iy)
		{
			...
			{
				RoutingNode n = new RoutingNode(new Point(ix, iy), tiles[iy][ix].getGround().getMoveCaps());
				nodes[iy][ix] = n;
			}
		}
